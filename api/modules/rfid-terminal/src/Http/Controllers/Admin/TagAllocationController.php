...
class TagAllocationController extends Controller
{
    ...
    {
        $tagFind = RfidTagAllocation::where('tag_data', $request->input('rfid_data'))->first();

        if($tagFind){
            return $this->respondWithError('tag_already_allocated', 422, 'Error: Tag is already allocated');
        }
