...
const { attendance, filters, meta } = storeToRefs(attendanceStore);
...
const pageSizes = ref([15, 30, 60, 100, 300, 500, 1000, 2000]);
const pageSize: Ref<number> = ref(15);

watch(pageSize, ()=>{
  attendanceStore.setPerPage(pageSize.value);
})
...
<template>
  <v-card-item>
    <v-row>
      <v-col cols="2">
        <v-select label="Level" :items="educationLevels" item-title="education_level_name" variant="outlined" v-model="selectedEduLevel" return-object density="compact"></v-select>
      </v-col>
      <v-col cols="2">
        <v-select label="Section" :items="currentSection" item-title="section_name" variant="outlined" v-model="selectedSection" return-object density="compact"></v-select>
      </v-col>
      <v-col cols="4">
        <v-btn>
      ...
      <v-col cols="4">
        <v-text-field variant="outlined" label="Search" v-model="filters.search" density="compact"></v-text-field>
      </v-col>
    </v-row>

    <div class="d-flex flex-row justify-space-between">
      <div style="width:100px!important">
        <v-select label="Per Page" variant="outlined" size="sm" :items="pageSizes" v-model="pageSize" density="compact"></v-select>
      </div>
      <p class="my-4 d-flex flex-row">Total Records: <b>{{ meta.total }}</b>&nbsp;|&nbsp; 
      Pages: <b>{{ meta.last_page }}</b>&nbsp;|&nbsp;
      Current Page: <b>{{ meta.current_page }}</b>
      </p>
    </div>

    <hr>

    <v-pagination :length="meta.last_page" :total-visible="10" v-model="filters.page" class="mt-5"></v-pagination>
